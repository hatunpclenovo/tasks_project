...
function toggleEmployeeOptions() {
...
function addEmployee(event) {
    ...
    newEmployee.innerHTML = `${employeeName} 
        <div class="task-counters">
            <span class="in-progress-counter">In Progress: 0</span>
            <span class="completed-counter">Completed: 0</span>
            <span class="uncompleted-counter">Uncompleted: 0</span>
        </div>`;
    newEmployee.onclick = () => viewEmployeeDetails(employeeName);
...
function loadEmployees() {
    ...
    employees.forEach(employee => {
        ...
        employeeItem.innerHTML = `${employee.name} 
            <div class="task-counters">
                <span class="in-progress-counter">In Progress: 0</span>
                <span class="completed-counter">Completed: 0</span>
                <span class="uncompleted-counter">Uncompleted: 0</span>
            </div>`;
        employeeItem.onclick = () => viewEmployeeDetails(employee.name);
        employeeList.appendChild(employeeItem);
        
        updateEmployeeCounters(employee.name);
    });
}
...
function enableDeleteEmployees() {
    const employeeList = document.getElementById('employeeList');
    Array.from(employeeList.children).forEach(li => {
        li.onclick = () => deleteEmployee(li);
        li.style.cursor = 'pointer';
        li.title = 'Click to delete';
    });
}

function deleteEmployee(employeeItem) {
    const employeeList = document.getElementById('employeeList');
    const section = getCurrentSection();
    let employees = JSON.parse(localStorage.getItem(section)) || [];
    employees = employees.filter(employee => employee.name !== employeeItem.textContent);
    localStorage.setItem(section, JSON.stringify(employees));
    employeeList.removeChild(employeeItem);
}

function enableEditEmployees() {
    const employeeList = document.getElementById('employeeList');
    Array.from(employeeList.children).forEach(li => {
        li.onclick = () => editEmployee(li);
        li.style.cursor = 'pointer';
        li.title = 'Click to edit';
    });
}

function editEmployee(employeeItem) {
    const newName = prompt('Enter new name:', employeeItem.textContent);
    if (!newName) return;

    const section = getCurrentSection();
    let employees = JSON.parse(localStorage.getItem(section)) || [];
    const employeeIndex = employees.findIndex(employee => employee.name === employeeItem.textContent);
    if (employeeIndex >= 0) {
        employees[employeeIndex].name = newName;
        localStorage.setItem(section, JSON.stringify(employees));
    }

    employeeItem.textContent = newName;
}
...
// Call loadEmployees when the page is loaded
